...
class GameEngine:
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_r and self.game_state == "fail":
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    self.boat.adjust_power(True)  # Increase power
                elif event.key == pygame.K_DOWN:
                    self.boat.adjust_power(False)  # Decrease power
    ...
    def update(self):
        ...
        # Handle mouse input for power control
        if pygame.mouse.get_pressed()[0]:  # Left mouse button
            mouse_pos = pygame.mouse.get_pos()
            if self.boat.power_up_region.collidepoint(mouse_pos):
                self.boat.adjust_power(True)
            elif self.boat.power_down_region.collidepoint(mouse_pos):
                self.boat.adjust_power(False)
    ...
    def _draw_islands(self):
        ...
        if (-100 <= island_screen_pos[0] <= self.settings.SCREEN_WIDTH + 100 and
            ...
            pygame.draw.circle(self.screen, (255, 255, 0, 100), island_screen_pos, 
                             self.settings.ISLAND_RADIUS + 10, 3)import pygame
import math
from game.boat import Boat
from game.wave import WaveGenerator
from game.player import Player
